...
class AirlineCodeController extends Controller
{
    ...
    public function addAirlineCode(Request $request)
    {
        $Airline='[
  {
    "id": "AUTOSTRAD",
  ...
  },
  {
    ...
    "logo": "https://images.kiwi.com/airlines/64/RS.png"
     ...
     $AirlineData= json_decode($Airline);

  

    $existingCarrierCodes = AirlineCode::pluck('carrier_code')->toArray();

    $beforeCount = AirlineCode::count();

    $data = [];
    foreach ($AirlineData as $Airline) {
        if (!in_array($Airline->id, $existingCarrierCodes)) {
            $data[] = [
                'carrier_code' => $Airline->id ?? null,
    ...
    AirlineCode::insert($data);
    
    $afterCount = AirlineCode::count();
